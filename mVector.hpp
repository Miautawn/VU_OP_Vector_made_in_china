...
class mVector
{
    ...
    public:
        ...
        mVector(InputIterator first, InputIterator last) { create(first, last); }

        //initializer konstruktorius
        mVector(std::initializer_list<T> initializer)
        { create(initializer.begin(), initializer.end()); }

        //copy konstruktorius
        mVector(const mVector& other) { create(other.begin(), other.end()); }
        ...
        mVector& operator=(std::initializer_list<T> initializer);
        ...
        reference at(size_type position) {
            ...
            } else { throw std::out_of_range("(mVector::at) out of range!!!"); }
        }
        ...
        iterator insert(iterator pos, size_type count, const T& value) {
            if(pos < begin() || pos > end()) throw std::out_of_range("(mVector::insert) out of range!!!");
        ...
        iterator erase(iterator first, iterator last) {
            if(first < begin() || last > end()) throw std::out_of_range("(mVector::insert) out of range!!!");
...
template <class T>
mVector<T>& mVector<T>::operator=(mVector&& other) {
    if(&other != this) {
        uncreate();
...
template <class T>
mVector<T>& mVector<T>::operator=(std::initializer_list<T> initializer) {
    uncreate();
    create(initializer.begin(), initializer.end());
    return *this;
}
...
template <class T>
void mVector<T>::assign(size_type count, const T& value) {
    uncreate();
    create(count, value);
}

template <class T>
void mVector<T>::assign(std::initializer_list<T> initializer) {
    uncreate();
    create(initializer.begin(), initializer.end());
}

template <class T>
void mVector<T>::reserve(mVector<T>::size_type new_cap) {
    if(new_cap > capacity()) {
        grow(new_cap, true);
    }
}
